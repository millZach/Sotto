...
export function summarizeVoice({ retrieval, capture, playback, speech, turns = [] } = {}) {
  const rows = []
  for (const phase of ['cold', 'warm']) {
    ...
    for (const [provider, label] of [['grok', 'Grok'], ['kokoro', 'Kokoro']]) {
      ...
      const trials = ((speech ?? playback)?.trials ?? []).filter(t => t.provider === provider && t.phase === (speech ? phase : phase === 'warm' ? 'screen' : 'cold'))
      const valid = trials.filter(t => t.ok && t.timestampErrors === 0 && t.discontinuities === 0 && t.clockUncertaintyMs < 2)
      rows.push(row(`${label} request to first loopback audio`, phase, valid.map(t => t.onsetMs), phase === 'warm' ? { p95: 300 } : null, `${speech ? 'Fresh' : 'Historical'} production playback/WASAPI; 300 ms is former on-device reference, not a new hosted release gate`))
      rows.push(row(`${label} playback interrupt to loopback silence`, phase, valid.filter(t => t.stopValid).map(t => t.stopMs), { max: 100 }, `${speech ? 'Fresh' : 'Historical'} production player stop; active-source/observed-silence validation`))
      if (speech) rows.push(row(`${label} injected Stop button input to loopback silence`, phase, valid.filter(t => t.stopValid).map(t => t.inputToSilenceMs), { max: 150 }, 'Fresh Electron mouse input -> trusted benchmark button click -> AgentVoiceSession.stopSpeaking -> NativeSystemSpeech -> Windows output silence; physical switch delay excluded'))
    }
