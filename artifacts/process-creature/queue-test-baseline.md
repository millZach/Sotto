...
The second scenario failed expecting the saved skill draft after reload but received an empty composer. A running-only guard initially passed but later reproduced the race: submit schedules a debounced save of the emptied composer. The direct fixture write now waits for accepted delivery, no thread command, no saved content draft, and a saved empty revision distinct from the delivered revision. This stronger guard preserved every assertion and passed five repeats on the unchanged baseline.
