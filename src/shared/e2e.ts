...
export interface SottoE2EBridge {
  agentEvent?(event: { type: 'ready' | 'manual' | 'question' | 'permission' | 'disconnect' | 'failure' | 'reasoner-release' | 'uncertain' | 'reject'; threadId: string; text: string; requestId?: string | undefined; status?: 'idle' | 'running' | 'error' | undefined }): Promise<void>
