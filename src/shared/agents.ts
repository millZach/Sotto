...
export const agentWorktreeSchema = z.object({
  ...
  /** What Start from origin found when the worktree was allocated: origin's branch was used, origin did not have
   * the branch so the local one was used, or the project has no origin remote so nothing was fetched. */
  originBase: z.enum(['used', 'missing', 'no-remote']).optional(),
