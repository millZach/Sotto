...
const personalDraftSchema = z.object({ revision, text: z.string().max(24000), skills: agentSkillReferencesSchema }).strict()
const personalSubmissionSchema = personalDraftSchema.extend({ id, messageId: id, status: z.enum(['submitting', 'accepted', 'uncertain', 'failed']), createdAt: z.string(), error: z.string().optional() })
