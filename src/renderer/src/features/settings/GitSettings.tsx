...
export function GitBehaviourSettings({ settings, onSave }: GitSettingsProps): ReactNode {
  return <>
    ...
    <Field label="Default merge method" description="The method a pull request's merge should start on. Last selected uses the one you chose last.">
    ...
    <Toggle label="Proactive panels" checked={settings.proactivePanels} onCheckedChange={checked => void onSave({ proactivePanels: checked })} description="Open Changes on its own after a turn that changed at least 3 files or 50 lines, when the Tools panel is closed. A turn in another pane waits until you go to its thread." />
  </>
}
