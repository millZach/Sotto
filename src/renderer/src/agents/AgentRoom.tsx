...
export function AgentRoom({ onOpenThreads, initialSheet = null }: { readonly onOpenThreads: () => void; readonly initialSheet?: 'session' | 'new' | null }): ReactNode {
  ...
  return <div className="agent-room">
    <div className="agent-room__tools">
      <Button variant="ghost" iconOnly aria-label="Connection settings" onClick={() => setSheet('settings')}><Settings2 size={17} /></Button>
    ...
    {sheet === 'session' ? <SideSheet title={active?.title ?? 'Session'} onClose={() => setSheet(null)}>
      {active ? <><p className="agent-muted">{project?.title} · {state.host.models.find(model => model.id === active.modelId)?.name ?? active.modelId}</p><AgentManualNotice state={state} command={command} />
        <div className="agent-actions">{assignment ? <><Button variant="ghost" onClick={() => void command({ type: assignment.paused || assignment.mode === 'manual' ? 'resume' : 'pause', threadId: active.id })}>{assignment.paused || assignment.mode === 'manual' ? 'Resume management' : 'Pause management'}</Button><Button variant="ghost" onClick={() => void command({ type: 'unassign', threadId: active.id })}>Stop managing</Button></> : <Button variant="secondary" disabled={!connected || state.busy || !supportsAgentSupervision(state.host.capabilities)} onClick={() => void command({ type: 'assign', threadId: active.id })}>Manage this thread</Button>}{active.status === 'running' && !isThreadClosed(active) && state.host.capabilities.interrupt ? <Button variant="ghost" disabled={!connected || state.busy} onClick={() => void command({ type: 'interrupt', threadId: active.id })}>Stop agent</Button> : null}</div>
