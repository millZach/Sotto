...
export function AgentAccountSettings(): ReactNode {
  ...
  return <div className="account-settings">
    <p className="settings-state">{configuration.reasoning === 'none' ? 'No reasoning account is connected. Choose the account Sotto uses for voice commands and follow-ups.' : `${account?.label ?? configuration.reasoning} is selected for Sotto’s reasoning. Provider usage is billed separately.`}</p>
...
export function AgentSetupFields(): ReactNode {
  ...
  return <div className="account-settings"><div className="account-rows">
    ...
    <label>Default agent model<select aria-label="Default agent model" value={configuration.defaultModelId} onChange={event => void save({ defaultModelId: event.target.value })}><option value="">Choose a model after connecting</option>{state.host.models.map(model => <option key={model.id} value={model.id} disabled={!model.ready}>{model.name}</option>)}</select></label>
