 ...
 */
export function BranchToolbar({ row, state, command, focused = true }: {
  ...
  const create = async (name: string): Promise<boolean> => {
    const worktree = thread.worktree
    if (worktree?.mode === 'independent' && !worktree.path) { setNotice({ text: 'A new worktree starts from a branch that exists. Choose one, or create the branch after the first send.', tone: 'error' }); return false }
  ...
  const hostName = thread.remoteHost ? thread.hostLabel ?? 'Remote host' : 'This device'
  const draftWorktree = thread.worktree?.mode === 'independent' && !thread.worktree.path
...
function WorkspaceMenu({ triggerRef, open, onOpenChange, label, value, options, disabled, onChoose }: {
  ...
  return <div className="branch-toolbar__menu"
    ...
    <button ref={triggerRef} type="button" className="branch-toolbar__chip tt-focusable" role="combobox" aria-label="Workspace" title={`Workspace: ${label}`} aria-haspopup="listbox" aria-expanded={open}
...
function BranchPicker({ threadId, triggerRef, open, onOpenChange, label, busy, disabled, draftWorktree, startFromOrigin, onPick, onCreate, onCopy, onStartFromOrigin }: {
  ...
  const canCreate = !draftWorktree && offersCreate(query, refs)
  const empty = !loading && refs.length === 0 && !canCreate
  ...
  return <div className="branch-toolbar__picker"
    ...
    onBlur={event => { if (open && !event.currentTarget.contains(event.relatedTarget as Node | null)) onOpenChange(false) }}>
    <button ref={triggerRef} type="button" className="branch-toolbar__branch tt-focusable" role="combobox" aria-label="Branch" title={busy ? 'Switching…' : label} aria-haspopup="listbox" aria-expanded={open}
      aria-controls={open ? panelId : undefined} disabled={disabled} data-busy={busy || undefined} onClick={() => onOpenChange(!open)}
    ...
    {open ? <div ref={panel} id={panelId} className="branch-toolbar__panel" role="group" aria-label="Branches">
        ...
        <input ref={search} aria-label="Search refs" placeholder="Search refs..." value={query} autoComplete="off" spellCheck={false}
          ...
          onKeyDown={event => {
            ...
            if (event.key === 'Enter') {
              ...
              const first = refs[0]
              if (first && !(canCreate && refs.length === 0)) void choose(first)
              else if (canCreate) { close(true); void onCreate(createName) }
            }
          }} />
      </div>
      <div ref={list} role="listbox" aria-label="Refs" className="branch-toolbar__refs" onKeyDown={event => moveListboxFocus(event, list.current)}
