 ...
 * while the agent streams into the message below it.
 */
const MessageArticle = memo(function MessageArticle({ message, provider, writing, streamText }: {
 ...
 */
const TurnView = memo(function TurnView({ turn, provider, running, last, writing, placement, context, streamText }: {
  ...
  const article = (message: AgentMessage): ReactNode => drawn(message)
    && <MessageArticle message={message} provider={provider} writing={message.id === writing} streamText={streamText} />
...
export const MessageList = memo(function MessageList({ messages, provider, running, placement, context, streamText = true }: {
  ...
  return <>{turns.map((turn, index) => <TurnView key={turn.key} turn={turn} provider={provider} running={running}
    last={index === turns.length - 1} writing={writing} placement={placement} context={context} streamText={streamText} />)}
