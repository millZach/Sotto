...
/** True while an overlay outside `inside` is open, so the native page steps aside for it. */
function useOverlayOpen(inside: React.RefObject<HTMLElement | null>): boolean {
  const [open, setOpen] = useState(false)
  useEffect(() => {
    const check = (): void => {
      const covering = [...document.querySelectorAll<HTMLElement>(OVERLAY_SELECTOR)].some(element => !inside.current?.contains(element) && element.getClientRects().length > 0)
      setOpen(covering)
    }
    check()
    const observer = new MutationObserver(check)
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['role', 'open', 'hidden', 'data-covers-native-view'] })
