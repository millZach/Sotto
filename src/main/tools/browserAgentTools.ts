...
const descriptions: Record<keyof typeof inputs, string> = {
  ...
  browser_open: 'Open a page for this thread in Sotto and begin a browser task. Use the actual URL of the app running in this thread\'s working copy; establish its server/port first, never guess another thread\'s port. The user must allow opening and sharing, once or for this thread; when the user has let this thread open pages it opens without waiting. Otherwise waits for the user answer and returns the executed result. If the wait expires, check browser_status before requesting again.',
