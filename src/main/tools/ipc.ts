...
export function registerToolsIpc(ipc: IpcMainAdapter, services: { terminal: TerminalService; browser: BrowserService; gitChanges: GitChangesService }, senders: () => readonly TrustedIpcSender[]): () => void {
  ...
  for (const method of ['list', 'diff', 'copyPath', 'reveal', 'watch', 'act', 'branches', 'checkpoints', 'inspectCheckpoint', 'revertCheckpoint', 'recoverCheckpoint', 'reviewPullRequest', 'draftPullRequestText', 'actPullRequest'] as const) register(GIT_CHANGES_CHANNEL + method, payload => services.gitChanges[method](payload))
  return () => {
