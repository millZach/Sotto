  ...
  return new NativeRuntimeController({
    ...
    registerIpc: () => {
      ...
      ipcMain.handle(AGENT_E2E, (event, payload: unknown) => {
        ...
        testAgentHost?.event(z.object({ type: z.enum(['ready', 'manual', 'question', 'permission', 'disconnect', 'failure', 'reasoner-release', 'uncertain', 'reject']), threadId: z.string(), text: z.string(), requestId: z.string().optional(), status: z.enum(['idle', 'running', 'error']).optional() }).strict().parse(payload))
      })
