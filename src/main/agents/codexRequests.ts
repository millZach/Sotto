...
const permissionMethods = ['item/commandExecution/requestApproval', 'item/fileChange/requestApproval', 'item/permissions/requestApproval']
const questionMethods = ['item/tool/requestUserInput', 'tool/requestUserInput', 'mcpServer/elicitation/request']
...
export function pendingRequest(id: string | number, method: string, value: unknown, sessionId: string, fileSummary?: string): CodexPendingRequest | undefined {
  if (![...permissionMethods, ...questionMethods].includes(method)) return
  const params = paramsSchema.parse(value)
  const permission = permissionMethods.includes(method)
  ...
  return { id, method, sessionId, params, request: { id: `rpc:${JSON.stringify(id)}`, kind: permission ? 'permission' : 'question', text, options } }
}

export function declineRequest(method: string): unknown {
  return method === 'item/permissions/requestApproval' ? { permissions: {}, scope: 'turn' }
    : method === 'mcpServer/elicitation/request' ? { action: 'decline', content: null }
      : questionMethods.includes(method) ? { answers: {} } : { decision: 'decline' }
}
