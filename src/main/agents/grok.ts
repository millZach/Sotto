  ...
  async connect(): Promise<AgentHostSnapshot> {
    ...
    try {
      await rpc.request('initialize', { protocolVersion: GROK_ACP_VERSION, clientCapabilities: { fs: { readTextFile: false, writeTextFile: false }, terminal: false }, clientInfo: { name: 'sotto', version: '1' } }, value => {
        const response = z.object({ protocolVersion: z.literal(GROK_ACP_VERSION), agentCapabilities: z.object({ loadSession: z.literal(true) }), authMethods: z.array(z.object({ id: z.string() })), _meta: z.object({ agentVersion: z.string().min(1).max(64), modelState: catalogSchema }) }).parse(value)
