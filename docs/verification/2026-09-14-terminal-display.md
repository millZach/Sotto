...
The exploratory run of older Phase 3 suites also encountered three failures outside the terminal behavior changed here: a tall-window working-folder wrap assertion, selected-file containment in the short-window Changes list (before the test reaches Terminal), and a 150px minimum personal-chat transcript assertion (observed 142.7px). These remain unresolved; this work does not claim the entire historical UI suite is green. Terminal-specific stale tab-order/settings-navigation/renderer selectors were updated.
