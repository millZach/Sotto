...
## Follow-up: new Codex thread transcript availability

Manual dev testing found a native lifecycle case absent from the earlier fixture: `thread/start` succeeds, but `thread/read` with `includeTurns: true` returns error -32600 until the first user message materializes the transcript. A direct native probe reproduced the exact rejection. The adapter now retries a metadata-only read only for that exact error and native thread ID on an empty, non-running thread. Revision checks, authored-message guards, and unrelated read errors remain enforced. The fixture reproduces the native rejection by default; the regression failed before the fix.

Restarting before that first message can also leave no saved Codex rollout. A second native probe reproduced the missing-rollout response. Missing sessions retain their aliases and expose a thread-specific history error, while healthy threads and new thread creation remain connected. The adapter never silently substitutes a new native session. This means a thread closed before its first message may require an explicit new thread; no saved provider transcript is available to resume.

Follow-up verification: 70 focused Codex/target-refresh/delivery/contract tests passed, with 2 existing skips; both TypeScript projects and focused ESLint passed. One concurrent run hit the existing 100 ms uncertain-settings fixture startup timeout; the final run passed. A fresh actual native Windows UI journey with Full access passed creation with no state error, first prompt, reply, and restart (11.2 seconds). The reply screenshot was visually inspected. The live user dev instance was reconnected successfully. Independent read-only review found zero material concerns. Earlier full-suite and visual-matrix counts above precede this follow-up; they were not rerun wholesale.
