...
PR #166 review identified long-session, ready-notice and cursor-resume edge cases. Bounded root/nested launch evidence replaces a permanent overflow latch, so fresh root monitors remain eligible after 4,096 launches while unknown ownership stays hidden. Retained subagent activity supplies task-type evidence after cursor resume. If a non-subagent start reuses a retained task ID, that history row records taskUpdatesExcluded; the schema and detail signature preserve it across restoration. An explicit ordinary start clears the exclusion without losing a terminal row to the merge guard. No monitor-ID cache or live monitoring state is persisted.

A ready notification no longer hides a live watch; permission, question and coordinator blocks still do. After integrating main at 886c0f26 (queued-message PR #165), typecheck, lint, notices and 108 focused tests passed. Build and all eight Electron scenarios across thread-monitoring, thread-workspace and queued-steering passed (26.5 seconds). The managed-ready regression preserves the sprite instance, draft and sending. The refreshed managed minimum capture was visually inspected.
