...
- The first PR CI run passed every Codex test and failed only the existing markdown-render stopwatch comparison. Incremental median time was 13.65 ms versus 27.66 ms and parsed characters were 28,973 versus 113,960, but total elapsed time reversed (1,726.7 ms versus 1,313.8 ms). Both renderer and benchmark matched `origin/main` before the correction. The elapsed-time comparison now follows the existing `SOTTO_PERF_ASSERT` opt-in policy; the deterministic parsed-work bound and render behavior remain active in CI.
- The second PR CI run exposed an ordering assumption in `nativeTargetRefresh.test.ts`: a stale response was released after 150 ms, independently of the live completion, and the completion flag checked text before the idle notification. The test now explicitly holds that response until both text and idle are observed, releases it, and requires a fresh history read. This corrects the scripted race without changing application code.
