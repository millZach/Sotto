...
One visual gap remains in the captured 760×850 conflict state: the recovery notice, thread header and local image composer push the composer model/permission/send controls below the footer (`local-draft-kept-760.png`). The unbound, direct recovery and cleared states fit; `bound-after-local-clear-760.png` shows the recovered composer fitting again. This clipping was reported for the parent layout pass and is not covered by the passing behavioral assertions.
