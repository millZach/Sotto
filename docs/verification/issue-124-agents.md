...
The next review identified an interruption window in the initial legacy migration. Normal completion was already safe because its final flush updated the marker; a failure before that flush could discard valid migrated activity on restart. Initial migration now commits legacy messages before activity. Three tests cover normal completion and failures before/after the activity import; all 62 focused recovery/store tests pass. The preceding complete suite passed 4,185 tests (34 skipped), and this narrow follow-up passes static gates and both rebuilt journeys in 10.7 seconds. The final full CI result is recorded on PR #168.