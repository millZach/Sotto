# Claude settings over the control channel - September 25, 2026
...
Run three more times with #318 in the same checkout, the adapter's figures held: 85.5-86.2 ms not running and 3.8-4.1 ms running, with the same starts and requests per press.
...
The opt-in live check (`tests/integration/claudeSettingsLive.test.ts`, `SOTTO_CLAUDE_LIVE=1`) ran the new path against the installed CLI four times, with no model turn; `docs/verification/2026-09-25-claude-settings-live.md` has the runs. For the old path on a running session it stops the thread's CLI through the adapter and then presses, which starts it again: the same stop, save and start the old `configure-thread` did.
