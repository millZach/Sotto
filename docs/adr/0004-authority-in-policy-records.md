...
- A thread's permission mode is the user's own choice in the thread options. "Ask for approval" keeps every provider request flowing to the user. "Allow edits", "Auto" and "Full access" let the provider act without raising some or all of those requests, so `always-confirm` boundaries are never consulted for actions it takes on its own. Codex offered these modes first; Claude and Grok offer them since 2026-09-14 (Grok has no "Allow edits"). Devin offers its own modes since 2026-09-22, each written into a Sotto-owned profile that decides what reaches the user; its default, "Ask first", keeps every request flowing to the user, and a thread made before then keeps it (ADR-0022).
