...
Sotto now does the same. `ThreadWorktrees.allocate` asks origin whether it has the branch before fetching (`git ls-remote --exit-code`), so a reachable remote that lacks the branch is told apart from a remote that cannot be reached. The worktree record keeps the answer as `originBase`: `used`, `missing` or `no-remote`. When the local branch was used, the pane says so once above the composer, in the status tone, with the branch named and a Dismiss; the branch toolbar shows the branch the folder is on as it always did. A fetch that fails for any other reason, the connection or the credentials, still refuses with the same words as before, and a base with no commit still refuses.
