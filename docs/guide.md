...
Claude Code model, effort and permission changes reach the thread's running session and wait for Claude Code to confirm them, so the session keeps going. When no session is running, when Claude Code refuses a change, or when the thread enters or leaves full access, Sotto starts the session again with the new settings instead. If Claude Code does not answer, Sotto stops that session and says the change is unconfirmed; the next start carries it, and nothing is sent twice.
