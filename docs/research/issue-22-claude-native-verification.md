...
Coding launch arguments are `--print --input-format stream-json --output-format stream-json --verbose --include-partial-messages --replay-user-messages --permission-mode <mapped mode> --permission-prompts host`, plus `--session-id <native UUID>` for an empty thread or `--resume <native UUID>` once the native transcript exists, `--model <installed alias>` and optional `--effort`. Coding processes retain tools, user configuration and native transcript persistence. The isolated reasoning client's tool-disabled flags are used only for its metadata discovery probe, never coding threads.

Tool answers preserve the native envelope. Explicit Allow sends `{behavior: 'allow', updatedInput: originalInput}`. Deny, missing approval, interruption and disconnect send `{behavior: 'deny', message: ...}`. No permission-rule changes or session-wide grants are synthesized. AskUserQuestion returns updated input with `answers` keyed by the exact question text. Malformed permission/question requests deny; unsupported control subtypes receive an error response. Every runtime mode is advertised and stored per thread: `approval-required` launches `--permission-mode default` (also the behaviour for threads with no stored mode), `auto-accept-edits` launches `acceptEdits`, `auto` launches `auto`, and `full-access` launches `bypassPermissions` together with `--allow-dangerously-skip-permissions`, which no other mode receives. Changing the mode restarts the native runtime. `--permission-prompts host` is kept for every mode, so any prompt the native mode still raises routes through Sotto. Checked against Claude Code 2.1.272 on 2026-09-14 with one short `--print` turn per mode, reading the `permissionMode` in the init event: `default`, `acceptEdits` and `bypassPermissions` start as requested; `auto` starts as `auto` on Sonnet and Opus but falls back to `default` on Haiku, so Haiku threads still ask for approval.
