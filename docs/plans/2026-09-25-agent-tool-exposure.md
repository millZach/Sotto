...
- **Thread-keyed state, one placement.** A player store keyed by Sotto thread ID, like T3's `previewMiniPlayerStore`, holds open, shrunk, hidden or docked. Position and size are one placement for the whole window, held for the session in the renderer, the way the Tools panel's own width is; neither is saved, so both start fresh on the next launch. It replaces the corner preview. The focused thread's player is drawn and no other.
...
- **Tests.** Unit tests for the store rules. A Playwright spec: an agent in thread A opens a page, the player shows over A; focus B, and neither A's page nor its player is drawn; go back to A and it returns. Keyboard: the player's controls have names, the drag bar and the corner grip both move and resize with the arrow keys, and Escape shrinks the player to its pill and returns focus to the composer.
...
### Step 4. Let Codex use its own tools (`fix/codex-computer-use-needs`, done September 25, 2026)
...
- **Not done: check for something narrower than Full access.** Still open (the verification note lists it as unchecked). Look for a Codex setting that lets the Computer Use process run under `workspace-write` (a sandbox exemption for one MCP server, or a writable root it needs). If one exists and is scoped to that server, prefer it, and re-run `SOTTO_CODEX_COMPUTER_USE_LIVE=1` to prove it.
- **Relay Codex's app-access questions for when they come.** Done for OpenAI's own form (`openai/form`, `openaiForm`), answered like a form, and for a `url` request, which shows its link with Continue and Decline. Not done for `item/permissions/requestApproval`: a bare approve there is refused on purpose, because its card already offers the two scopes Codex asks about and Sotto should not pick one (`codexHost.test.ts` holds that).
- **Dropped: stop overriding the policy at the process level.** Full access worked with the process-level flags in place, so removing them would change nothing the user sees and risks regressions.
- **Show Computer Use as itself.** Done from the server and the code rather than a surface marker: codex app-server 0.157.0's `mcpToolCall` item carries none. `cua_repl` calls, and `node_repl` calls that use `@oai/sky`, are titled Computer Use; the code itself is still not kept.
...
- **Tests.** `tests/unit/main/codexComputerUse.test.ts` (naming, the two plain failures, the link and OpenAI-form relays). Live: `SOTTO_CODEX_COMPUTER_USE_LIVE=1` with the `$computer-use` prompt showed the sandbox message in the default mode and the Codex-app message in Full access with the app closed.
