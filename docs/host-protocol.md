...
A host whose health carries no `features` predates the freeze, and one with another `v` speaks another protocol. The desktop sends such a host nothing more and says: "This host is running a different version of Sotto. Nothing on the host was lost. Stop host, then connect again to start the new version." For a host Sotto did not start, the sentence says to stop it on that machine instead. The Hosts page keeps Stop host beside the sentence for a host Sotto started.
