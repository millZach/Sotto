...
A thread with its own working copy follows whatever branch that folder has checked out. The branch label in the pane header is re-read after a turn, when you start typing and when the window comes back into focus, so a switch made in a terminal shows without a send. When the branch differs from the one the last send went to, a **Branch changed** notice sits above the composer once there is text in it; **Restore branch** switches the folder back, asking first when it has uncommitted changes, and **Dismiss** hides the notice until the branch moves again. A working folder that was deleted by hand is put back on its recorded branch before the next send, with the branch's own commits; nothing is reset or removed, and the send is refused only when that branch is checked out in another folder.
...
A thread's messages are Sotto's own record, kept in an event store rather than rebuilt from the provider each time Sotto starts. A pane opens on the newest ten turns and reads further back when you ask for earlier messages, so a long thread opens as quickly as a short one, and a provider session starts when a thread is opened or sent to rather than at connect. **Keep local history** off still means no message text is written at all, and turning it off removes what was written; what was not kept cannot be recovered. See ADR-0016.
