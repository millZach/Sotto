...
5. For natural-language project/thread commands and automatic directional answers or fixes, select **Sotto reasoning**. Choose an existing **ChatGPT subscription through Codex** or **Claude subscription through Claude Code**, check the connection, choose an offered model or the supported default, and save. Sign into the provider’s own installed client first; Sotto never asks you to paste its login tokens. Explicit OpenRouter and OpenAI API accounts remain available as alternatives. Manual controls, prompt dictation, and local control phrases also work without reasoning.
...
The native subscription clients retain authentication and refresh their own logins. Sotto checks the actual account before requesting a decision, disables tools and executable customizations in the reasoning subprocess, and validates the returned decision before its controller can act. Missing clients, unsupported versions/models, expired sign-ins, and usage errors stay on the selected route and produce a visible error. Changing a reasoning route clears the previous Sotto reasoning API key; it does not change logins in the provider apps.

Grok subscription reasoning is explicitly unavailable in this build. The inspected Grok Build client does not yet provide a verified combination of subscription-only authentication and isolation from native startup hooks/MCP. This does not remove Grok from T3’s own coding-provider connections. See the [provider investigation](verification/subscription-reasoning-routes-2026-09-09.md) and [subscription QA record](verification/subscription-reasoning-qa.md) for exact supported clients and verification limits.
