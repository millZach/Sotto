...
5. For automatic directional answers and fixes, choose a Sotto reasoning API and model, then save its API key. Human decisions only leaves decisions in the queue. Coding subscriptions in T3 do not automatically provide Sotto reasoning API access.
...
Select a project and create a thread with the default model or an explicit available override. Select **Manage** to assign an existing thread. A thread is monitored only after assignment; selecting an existing thread does not authorize automatic replies.
...
The queue names the project and thread needing attention. A spoken answer becomes an editable **Answer draft** bound to that question. Pauses do not send it; say **send it** when finished. The question binding survives a restart. You can also type into the question's answer box and select **Answer question**. Say **Next** or **Later** to move on before composing, or send/clear a draft first. Select a named thread to change the presented item. Skipping a permission never approves it. Say **allow** or **deny**, or use its explicit approval controls. Sotto does not override T3’s own permission system.
