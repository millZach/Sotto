...
it.each([false, true])('finishes a Claude subagent after provider/workspace restart with activity eviction=%s', async evicted => {
  let fixture = await claudeFixture(undefined, 15_000)
  let workspace = new WorkspaceHost(fixture.host, fixture.root)
  ...
  try {
    ...
    workspace = new WorkspaceHost(fixture.host, root)
    await workspace.connect()
    ...
    expect(assignments.assignments[0]).toMatchObject({ id: active.assignmentId, prompt: 'PRIVATE_TASK_PROMPT', result: 'Fresh completion after restart', status: 'completed' })
    ...
    expect(after.rows.find(row => row.id === completed.id)?.status).toBe('completed')
    ...
    expect((await workspace.subagentPage({ threadId: 'thread' })).rows).toHaveLength(2)
    await fixture.action('thread', { type: 'raw-burst', frames: [
    ...
    expect(history.assignments[1]).toMatchObject({ id: active.assignmentId, result: 'Fresh completion after restart', status: 'completed' })
  } finally {
    workspace.disconnect()
    await fixture.adapter.closed()
    await workspace.privacyChanged()
