...
import type { RequestDraftOwner } from '../../src/shared/requestDrafts'
...
it.each([true, false])('binds threaded disk recovery to positive acceptance (%s), never request disappearance alone', async accepted => {
  ...
  await drafts.start()
  f.host.event({ type: 'question', threadId: owner.ownerId, text: '', request })
  await drafts.save({ target, revision: 1, held: true, selections: { q: { optionIds: [], other: false, text: 'Private held answer' } } })
  ...
  await f.command({ type: 'answer', threadId: owner.ownerId, requestId: request.id, answer: '', questionAnswers: { q: { optionIds: [], text: 'Private held answer' } } })
