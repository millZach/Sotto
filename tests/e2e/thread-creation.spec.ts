...
test('creates a thread in a centered popup, configures it, and sends file and pasted screenshots', async () => {
  ...
  try {
    ...
    await expect(dialog).toBeVisible()
    await page.screenshot({ animations: 'disabled', path: 'artifacts/new-thread-setup/new-thread-provider-models.png' })
    await page.keyboard.press('Escape')
    await expect(page.getByRole('dialog', { name: 'Choose model' })).toHaveCount(0)
    await expect(dialog).toBeVisible()
    if (!await dialog.locator('details').evaluate(node => node.hasAttribute('open'))) await dialog.locator('summary').click()
    ...
    await page.screenshot({ animations: 'disabled', path: 'artifacts/new-thread-setup/new-thread-options.png' })
    await dialog.getByRole('button', { name: 'Create thread' }).click()
    await expect(dialog).toHaveCount(0)
