...
test('Crossing keeps dictation, history, settings and sessions usable', async () => {
  ...
  try {
    ...
    await page.getByRole('link', { name: 'AI account', exact: true }).click()
    await page.screenshot({ animations: 'disabled', path: 'artifacts/crossing/account.png' })
    await page.getByLabel('Reasoning account', { exact: true }).selectOption('openrouter')
    await page.getByLabel('Reasoning API key', { exact: true }).fill('fixture-only-key')
    await page.getByLabel('Reasoning API key', { exact: true }).press('Tab')
    await expect(page.getByRole('button', { name: 'Remove reasoning API key', exact: true })).toBeVisible()
    await page.getByRole('button', { name: 'Remove reasoning API key', exact: true }).click()
    await expect.poll(() => page.evaluate(async () => (await window.sotto!.agents!.get()).credentials.reasoning)).toBe(false)
    await page.getByLabel('Reasoning account', { exact: true }).selectOption('none')
    ...
    await expect(page.getByRole('dialog', { name: 'New thread', exact: true })).toBeVisible()
    await page.getByRole('button', { name: /Sotto test C:\/sotto-test/ }).click()
