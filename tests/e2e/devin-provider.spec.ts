  ...
  try {
    ...
    for (const [width, height] of [[1600, 1000], [1280, 800], [820, 560]] as const) {
      ...
      for (const appearance of ['dark', 'light'] as const) {
        ...
        const ratio = await textContrast(panel.locator('.provider-models__empty'))
    ...
    await panel.getByRole('combobox', { name: 'Devin default thread model' }).selectOption({ label: 'Fixture Devin' })
