...
test('Index settings: focused categories, real saves, failure feedback, themes and persistence in Windows', async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Use Fern dark mode', exact: true }).click()
    await expect(page.locator('html')).toHaveAttribute('data-theme-id', 'grove')
    await expect.poll(async () => (await settings()).darkTheme).toBe('grove')
    await shot(page, 'appearance-fern-live')
    await page.getByRole('button', { name: 'Use Fern theme', exact: true }).focus()
    await expect(page.getByRole('button', { name: 'Duplicate Fern', exact: true })).toBeVisible()
    await shot(page, 'appearance-keyboard-actions')
    ...
    await writeFile(join(evidence, 'verification.json'), JSON.stringify({ layout, errors, persisted: { pasteDelayMs: 325, dictionary: true, darkTheme: 'grove' } }, null, 2))
