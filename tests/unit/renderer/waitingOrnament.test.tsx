...
import { useWaitingAction } from '../../../src/renderer/src/agents/ThreadMonitor'
import { WAITING_AFTER_MS } from '../../../src/renderer/src/agents/threadActivityView'
...
describe('the waiting ornament’s own clock', () => {
  it('says nothing until the threshold passes, then says so without another event arriving', () => {
    const live = thread(WAITING_AFTER_MS - 2_000)
    const view = renderHook(({ value }: { value: LiveThread }) => useWaitingAction(value, true, undefined),
  ...
  it('answers immediately for an action that was already past the threshold when it arrived', () => {
    const view = renderHook(() => useWaitingAction(thread(WAITING_AFTER_MS + 1_000), true, undefined))
    expect(view.result.current?.id).toBe('action')
  })

  it('restarts the wait when a new action replaces the one being waited on', () => {
    const view = renderHook(({ value }: { value: LiveThread }) => useWaitingAction(value, true, undefined),
      { initialProps: { value: thread(WAITING_AFTER_MS + 1_000) } })
    expect(view.result.current?.id).toBe('action')
    view.rerender({ value: thread(0) })
    expect(view.result.current).toBeUndefined()
    act(() => { vi.advanceTimersByTime(WAITING_AFTER_MS) })
    expect(view.result.current?.id).toBe('action')
  })

  it('says nothing while it is ineligible, and schedules nothing against a held clock', () => {
    const live = thread(WAITING_AFTER_MS - 2_000)
    const blocked = renderHook(() => useWaitingAction(live, false, undefined))
    act(() => { vi.advanceTimersByTime(WAITING_AFTER_MS) })
    expect(blocked.result.current).toBeUndefined()
    expect(vi.getTimerCount()).toBe(0)

    const held = renderHook(() => useWaitingAction(live, true, origin))
    expect(held.result.current).toBeUndefined()
    expect(vi.getTimerCount()).toBe(0)
    expect(renderHook(() => useWaitingAction(live, true, origin + 2_000)).result.current?.id).toBe('action')
  })

  it('drops the ornament when the action ends, and clears its timer with it', () => {
    const view = renderHook(({ value }: { value: LiveThread }) => useWaitingAction(value, true, undefined),
      { initialProps: { value: thread(WAITING_AFTER_MS - 5_000) } })
