...
describe('AgentRequestCard', () => {
  it('renders every question with its real choices and sends one structured answer by keyboard', async () => {
    ...
    await user.click(screen.getByRole('textbox', { name: 'Other answer to: Which database?' }))
...
describe('composer question choices', () => {
  it('keeps a dedicated custom answer through choice changes and collapse, and sends only the explicit choice', async () => {
    ...
    const store = new RequestAnswerStore()
    const question = { ...single, options: [{ id: 'pg (Recommended)', label: 'Postgres (Recommended)' }, { id: 'lite', label: 'SQLite' }] }
    render(<AgentRequestCard placement="composer" ownerId="thread-a" ownerTitle="Workshop" request={structured([question])}
      blocked={null} onSubmit={onSubmit} store={store} />)
    const custom = screen.getByRole('textbox', { name: 'Other answer to: Which database?' })
    const choice = screen.getByRole('radio', { name: 'Postgres (recommended)' })
    const send = screen.getByRole('button', { name: 'Send answer' })
    expect(custom).toBeVisible()
    expect(choice).not.toBeChecked()
    expect(send).toBeDisabled()
    choice.focus()
    await user.keyboard('{ArrowDown}{ArrowDown}')
    expect(screen.getByRole('radio', { name: 'Write my own answer' })).toHaveFocus()
    ...
    await user.type(custom, 'DuckDB{Enter}for local use')
    expect(custom).toHaveValue('DuckDB\nfor local use')
    expect(screen.getByRole('radio', { name: 'Write my own answer' })).toBeChecked()
    expect(onSubmit).not.toHaveBeenCalled()
    ...
    expect(custom).not.toBeVisible()
    const reopen = screen.getByRole('button', { name: 'Show question' })
    expect(reopen).toHaveFocus()
    await user.keyboard('{Enter}')
    expect(custom).toBeVisible()
    expect(choice).toBeChecked()
    await user.click(send)
    expect(onSubmit).toHaveBeenCalledExactlyOnceWith({ answer: '', questionAnswers: { 'q-db': { optionIds: ['pg (Recommended)'] } } })
  })

  it('does not invent a recommendation and makes legacy choices wait for Send answer', async () => {
    const { user, onSubmit } = setup({ id: 'legacy-question', kind: 'question', text: 'Choose a database', options: [{ id: 'sqlite', label: 'SQLite' }] })
    expect(screen.queryByText('(recommended)')).not.toBeInTheDocument()
    await user.click(screen.getByRole('radio', { name: 'SQLite' }))
    expect(onSubmit).not.toHaveBeenCalled()
    await user.click(screen.getByRole('button', { name: 'Send answer' }))
    expect(onSubmit).toHaveBeenCalledExactlyOnceWith({ answer: 'sqlite' })
  })
})
