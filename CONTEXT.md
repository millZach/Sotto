...
**Host service.** The interface the host offers a client and nothing more: the event stream after a sequence number, the shell, one thread's detail, and commands. `HostService` in `src/main/agents/hostService.ts`, with `LocalHostService` over `AgentControl` as the only implementation today; the window's commands go through it, so a second transport is a new caller rather than a rewrite. Distinct from `AgentHost`, which is the interface a provider adapter implements on the other side of the coordinator. Avoid: "API", "server interface".
...
**Option chips.** The three controls in the composer footer that say what a thread is set to: the model, with its provider's mark, the reasoning effort and the permissions. Each is one chip that opens its own list above the composer, so the typed text and the send disc stay where they were. The model's list has provider tabs across the top, a search line and, on a thread before its first message, the reminder that any provider is still a choice; once the thread has sent, the list holds its own provider's models alone. The effort and permission lists come from the provider: Claude Code reports its effort levels at initialize, so a level it does not report cannot be offered. New thread and New terminal show the same three controls laid out in full. Avoid: "pill" or "Thread options" for the row.
