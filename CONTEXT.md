...
**Attention queue.** The coordinator's ordered list of the threads it manages that need the user: a thread is `ready` for a prompt, has a `question`, has a `permission` request, or is `blocked`. It orders what the coordinator presents and says; it is not where a thread's waiting is recorded. A thread you run yourself never enters it, and its sidebar row and composer still say it **waits on you** from the thread's own pending requests. Permissions are never answered automatically and are never inferred. Avoid: "inbox", "notifications".
