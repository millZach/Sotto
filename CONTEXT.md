...
**Commit message draft.** The message a side call to the thread's own provider writes for a Git action whose commit dialog left the message empty, from the staged diff, the staged file names, the repository's last twenty commit subjects and its `AGENTS.md`, with an imperative subject under 72 characters and a body only where the change needs one. A message typed in the dialog is used as typed. With generated commit messages off, on a Devin thread, or when the provider does not answer, the commit takes the subject "Update project files".

**Pull request draft.** The title and body a side call to the thread's own provider drafts when the pull request form opens on a branch with commits, from the branch's commit subjects and its capped diff against the base alone. It replaces the form's prefill from the last commit, not words the user has typed; the user edits, clears or regenerates it, and only the Create button creates anything. With generated pull request text off, on a Devin thread, or when the provider does not answer, the form opens as it always did.
...
**Branch toolbar.** The row under a thread's composer for a Git repository, as T3 Code has it (ADR-0027): **Run on** and **Workspace** at the left, the pull request badge and the **branch picker** at the right. Run on names the machine the thread runs on and is always static text here, since the host is chosen when the thread is made. Workspace is where a draft will work: **Current checkout**, **New worktree**, or **Previous worktree (branch)** for a worktree another thread of the project already has; it locks to static text once the thread has a message or a session. Avoid: "git bar", "workspace picker".

**Branch picker.** The branch toolbar's ghost button naming the folder's branch, **Select ref** for a detached HEAD, or **From origin/main** for a worktree not made yet. Its popover searches the host's refs, lists locals then remotes with `current`, `worktree`, `remote` and `default` badges, and pages them. Picking a ref checks it out in the thread's folder (a remote ref gets a tracking branch); a ref already checked out in another worktree re-points a draft at that worktree; for a worktree not made yet the pick records the base, with a **Start from origin** switch; a name nothing matches offers **Create new ref**, except for a worktree not made yet, which starts from a branch that exists. A draft pointed at another thread's worktree shows that worktree's branch and does not move it before the first send. Right-click copies the name. Git's refusal is shown under the row as **Failed to switch ref.** with Git's own words. A switch made here moves the thread's sent branch with it, so the branch-changed notice stays for checkouts moved elsewhere. Avoid: "branch dropdown", "checkout menu".
...
**Background work.** Agent work started from this thread that the provider has confirmed is still running, whether or not a turn is: a workflow, a subagent, a teammate or a remote agent. The creature stands at the end of the track by the readout and sends small agents out along it, and the reader is told **Working**, or *Working · 3 agents* when there are several. The evidence is the provider's own task frames, a start and the bookend that ends it, never elapsed time or what the transcript says; today only Claude Code reports them. A subagent the turn is still blocking on is the turn's own action, not background work, until it is moved to the background. Work started by another agent is that agent's business and does not count. Background work survives the end of the turn, keeps its provider session from being reaped, and ends with its bookend, an interrupt, an error, a disconnect or a restart. Like a monitoring task it is observation only, grants nothing and is never restored from history. A monitoring task outranks it; it outranks a held action. Not to be confused with **working preferences**, your answers about how you like to work.
...
**Askpass helper.** The small program OpenSSH runs through `SSH_ASKPASS` when it needs a password, a key passphrase or a yes to a new host key. It carries the question to the desktop over a loopback-only socket, the desktop shows it in the Hosts page, and the helper hands the answer back to ssh. An answer is reused for the rest of that connection, so the user answers once per connect. A notice OpenSSH runs it for without asking anything, such as a security key's touch request, is not shown, and a question ssh stops waiting for leaves the Hosts page. `src/main/hosts/sshAskpass.ts`. Avoid: "SSH prompt handler", "password dialog".
