...
**Askpass helper.** The small program OpenSSH runs through `SSH_ASKPASS` when it needs a password, a key passphrase or a yes to a new host key. It carries the question to the desktop over a loopback-only socket, the desktop shows it in the Add host dialog, or on the Hosts page for a saved host, and the helper hands the answer back to ssh. An answer is reused for the rest of that connection, so the user answers once per connect. A notice OpenSSH runs it for without asking anything, such as a security key's touch request, is not shown, and a question ssh stops waiting for leaves the Hosts page. `src/main/hosts/sshAskpass.ts`. Avoid: "SSH prompt handler", "password dialog".
