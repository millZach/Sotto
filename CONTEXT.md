...
**Commit message draft.** The message a side call to the thread's own provider drafts from the staged diff alone when the Changes panel's commit form opens, with an imperative subject under 72 characters and a body only where the change needs one. It is a draft: the user edits, clears or regenerates it, and only the Commit button commits it. With generated commit messages off, on a Devin thread, or when the provider does not answer, the form opens empty.
