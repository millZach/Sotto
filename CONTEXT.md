...
**Askpass helper.** The small program OpenSSH runs through `SSH_ASKPASS` when it needs a password, a key passphrase or a yes to a new host key. It carries the question to the desktop over a loopback-only socket, the desktop shows it in the Hosts page, and the helper hands the answer back to ssh. An answer is reused for the rest of that connection, so the user answers once per connect. `src/main/hosts/sshAskpass.ts`. Avoid: "SSH prompt handler", "password dialog".
