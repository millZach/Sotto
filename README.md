...
Because a host outlives updates to this computer, the two can run different versions of Sotto. That is fine while they speak the same host protocol ([docs/host-protocol.md](docs/host-protocol.md)). When they do not, the host's row says: "This host is running a different version of Sotto. Nothing on the host was lost. Stop host, then connect again to start the new version." Put the matching host archive in the host installation folder first, because Connect starts whatever is installed there. **Stop host** stays on the row for a host Sotto started; a host you started yourself you stop on that machine.
