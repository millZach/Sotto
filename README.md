...
Sotto asks before trusting a new host key, and for a password or key passphrase when SSH needs one: in the Add host dialog while you add a host, and on the Hosts page when a saved host reconnects. You answer once per connect, although a connection runs several SSH commands. What you type goes only to ssh on this computer, is kept in memory until the connection closes, and is never written down or logged.
