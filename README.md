...
- **"OpenRouter error" or "Couldn't transcribe":** OpenRouter's transcription service failed, or its answer could not be read. Dictate again in a moment. If it keeps happening, `transcription-diagnostics.jsonl` in Sotto's data folder (`%APPDATA%\sotto` on Windows) records each failure's reason and HTTP status, without any of your words.
